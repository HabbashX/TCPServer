...
public final class DefaultAuthentication extends Authentication {
    ...
    /**
     * Represents the file storage used for managing and persisting authentication data.
     * This variable is a critical component of the DefaultAuthentication class,
     * enabling support for file-based authentication mechanisms such as CSV and JSON.
     * It is initialized during the construction of the DefaultAuthentication instance and remains immutable.
     */
    private final File authenticationFileStorage;
    /**
            ...
            if (!isUserExists(username)) {
                ...
                if (!authenticationFileStorage.exists()) {
                    if (!authStorageType.equals(AuthStorageType.SQL)) {
                        ...
                        try {
                            authenticationFileStorage.createNewFile();
                        } catch (IOException e) {
    ...
    private void csvRegister(String username, String password, String email, String phoneNumber, @NotNull UserHandler userHandler) {
        ...
        if (!isUserExists(username)) {
            try (final CSVPrinter printer = new CSVPrinter(new BufferedWriter(new FileWriter(authenticationFileStorage)), format)) {
    ...
    private void jsonRegister(String username, String password, String email, String phoneNumber, @NotNull UserHandler userHandler) {
        ...
        if (!isUserExists(username)) {
            try {
                ...
                final List<Map<String, Object>> list = mapper.readValue(authenticationFileStorage, new TypeReference<>() {
                });

                list.add(map);
                mapper.writerWithDefaultPrettyPrinter().writeValue(authenticationFileStorage, list);
    ...
    private void jsonLogin(String username, String password, UserHandler userHandler) {
        ...
        try {
            if (!isUserConnected(username)) {
                final List<Map<String, Object>> users = mapper.readValue(authenticationFileStorage, new TypeReference<>() {
                });
